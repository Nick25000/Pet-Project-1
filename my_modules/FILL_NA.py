...
class FillNa:
    def __init__(self, dataset):
        ...
        # times_columns
        self.home_team_history_match_dates = ['match_date']+[f'home_team_history_match_date_{i}' for i in range(1,11)]
        self.away_team_history_match_dates = ['match_date']+[f'away_team_history_match_date_{i}' for i in range(1,11)]
    ...
    def fill_na_goal_team(self):
        ...
        [self.dataset[i].fillna(self.dataset['home_team_history_goal_1'], inplace=True) for i in self.home_team_history_goal]
        [self.dataset[i].fillna(self.dataset['away_team_history_goal_1'], inplace=True) for i in self.away_team_history_goal]
        
        self.dataset[self.home_team_history_goal+self.away_team_history_goal] = self.dataset[self.home_team_history_goal+self.away_team_history_goal].astype('int64')
    ...
    def fill_na_goal_team_opponent(self):
        ...
        mean_goal = int(self.dataset[self.home_team_history_opponent_goal+self.away_team_history_opponent_goal].mean().mean())
        columns = [f'goal_{i}' for i in range(1,11)]
        split_index = self.dataset.shape[0]
        ...
        [self.dataset[i].fillna(self.dataset['home_team_history_opponent_goal_1'], inplace=True) for i in self.home_team_history_opponent_goal]
        [self.dataset[i].fillna(self.dataset['away_team_history_opponent_goal_1'], inplace=True) for i in self.away_team_history_opponent_goal]
        
        self.dataset[self.home_team_history_opponent_goal+self.away_team_history_opponent_goal] = self.dataset[self.home_team_history_opponent_goal+self.away_team_history_opponent_goal].astype('int64')
        assert self.dataset[self.home_team_history_opponent_goal+self.away_team_history_opponent_goal].isna().sum().sum() == 0
    ...
    def fill_na_rating_team(self, to_log = True, standardization = False):
        ...
        [self.dataset[i].fillna(self.dataset['home_team_history_rating_1'], inplace=True) for i in self.home_team_history_rating]
        [self.dataset[i].fillna(self.dataset['away_team_history_rating_1'], inplace=True) for i in self.away_team_history_rating]
    ...
    def fill_na_rating_team_opponent(self,to_log = True, standardization = False):
        ...
        #away_rating['tag'] = 'away'
        away_rating.rename(columns = {i:j for i,j in zip(self.away_team_history_opponent_rating, columns)}, inplace=True)
        away_rating.rename(columns = {'away_team_name':'team_name'}, inplace=True)
        away_rating.reset_index(drop=True, inplace=True)
        ...
        [self.dataset[i].fillna(self.dataset['home_team_history_opponent_rating_1'], inplace=True) for i in self.home_team_history_opponent_rating]
        [self.dataset[i].fillna(self.dataset['away_team_history_opponent_rating_1'], inplace=True) for i in self.away_team_history_opponent_rating]
