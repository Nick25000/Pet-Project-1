...
class FeaturesEngineering:
    ...
    def Ft_eng_match_date(self):
        ...
        for i in range(10):
            self.dataset[f'home_team_days_betw_games_{i + 1}'] = (
                        self.dataset[match_dates_home[i]] - self.dataset[match_dates_home[i + 1]]).dt.days
            self.dataset[f'away_team_days_betw_games_{i + 1}'] = (
                        self.dataset[match_dates_away[i]] - self.dataset[match_dates_away[i + 1]]).dt.days
        ...
        for i in range(1, 11):
            self.dataset[f'home_diff_date_{i}'] = (
                        self.dataset['match_date'] - self.dataset[f'home_team_history_match_date_{i}']).dt.days
            self.dataset[f'away_diff_date_{i}'] = (
                        self.dataset['match_date'] - self.dataset[f'away_team_history_match_date_{i}']).dt.days

        # last game was during last week - [True, False]
        self.dataset['home_team_last_game_this_week'] = self.dataset['home_team_days_betw_games_1'].apply(
            lambda x: 1 if x <= 7 else 0)
        self.dataset['away_team_last_game_this_week'] = self.dataset['away_team_days_betw_games_1'].apply(
            lambda x: 1 if x <= 7 else 0)
    ...
    def Ft_eng_history_goal(self):
        ...
        self.dataset['home_goals_scored'] = self.dataset[[f'home_team_history_goal_{i}' for i in range(1, 11)]].sum(
            axis=1)
        self.dataset['home_goals_conceded'] = self.dataset[
            [f'home_team_history_opponent_goal_{i}' for i in range(1, 11)]].sum(axis=1)
        self.dataset['away_goals_scored'] = self.dataset[[f'away_team_history_goal_{i}' for i in range(1, 11)]].sum(
            axis=1)
        self.dataset['away_goals_conceded'] = self.dataset[
            [f'away_team_history_opponent_goal_{i}' for i in range(1, 11)]].sum(axis=1)
        ...
        for i in range(1, 11):
            self.dataset[f'home_team_win_{i}'] = (self.dataset[f'home_team_history_goal_{i}'] - self.dataset[
                f'home_team_history_opponent_goal_{i}']).apply(lambda x: function(x))
            self.dataset[f'away_team_win_{i}'] = (self.dataset[f'away_team_history_goal_{i}'] - self.dataset[
                f'away_team_history_opponent_goal_{i}']).apply(lambda x: function(x))

        # team point for last 10 games
        self.dataset['home_team_points'] = self.dataset[[f'home_team_win_{i}' for i in range(1, 11)]].apply(
            lambda x: x.map({1: 3, 2: 1, 0: 0})).sum(axis=1)
        self.dataset['away_team_points'] = self.dataset[[f'away_team_win_{i}' for i in range(1, 11)]].apply(
            lambda x: x.map({1: 3, 2: 1, 0: 0})).sum(axis=1)

        # count of wins, loses and draws for the last 10 days
        self.dataset['home_team_count_win'] = self.dataset[[f'home_team_win_{i}' for i in range(1, 11)]].apply(
            lambda x: len(x[x == 1]), axis=1)
        self.dataset['away_team_count_win'] = self.dataset[[f'away_team_win_{i}' for i in range(1, 11)]].apply(
            lambda x: len(x[x == 1]), axis=1)

        self.dataset['home_team_count_lose'] = self.dataset[[f'home_team_win_{i}' for i in range(1, 11)]].apply(
            lambda x: len(x[x == 0]), axis=1)
        self.dataset['away_team_count_lose'] = self.dataset[[f'away_team_win_{i}' for i in range(1, 11)]].apply(
            lambda x: len(x[x == 0]), axis=1)

        self.dataset['home_team_count_draw'] = self.dataset[[f'home_team_win_{i}' for i in range(1, 11)]].apply(
            lambda x: len(x[x == 2]), axis=1)
        self.dataset['away_team_count_draw'] = self.dataset[[f'away_team_win_{i}' for i in range(1, 11)]].apply(
            lambda x: len(x[x == 2]), axis=1)
        ...
        self.dataset['home_team_streak_wins'] = self.dataset[[f'home_team_win_{i}' for i in range(1, 11)]].apply(
            lambda x: funct(x), axis=1)
        self.dataset['away_team_streak_wins'] = self.dataset[[f'away_team_win_{i}' for i in range(1, 11)]].apply(
            lambda x: funct(x), axis=1)
        ...
        self.dataset['home_team_streak_loses'] = self.dataset[[f'home_team_win_{i}' for i in range(1, 11)]].apply(
            lambda x: funct_lose(x), axis=1)
        self.dataset['away_team_streak_loses'] = self.dataset[[f'away_team_win_{i}' for i in range(1, 11)]].apply(
            lambda x: funct_lose(x), axis=1)
    ...
    def Ft_eng_win_percentage(self):
        ...
        for indx in tqdm(self.dataset.index):
            summ = 0
            for i in range(1, 11):
                if self.dataset.loc[indx, f'home_team_win_{i}'] == 1 and self.dataset.loc[
                    indx, f'home_team_history_is_play_home_{i}'] == 1:
                    summ += 1
        ...
        for indx in tqdm(self.dataset.index):
            summ = 0
            for i in range(1, 11):
                if self.dataset.loc[indx, f'away_team_win_{i}'] == 1 and self.dataset.loc[
                    indx, f'away_team_history_is_play_home_{i}'] == 1:
                    summ += 1
        ...
        for indx in tqdm(self.dataset.index):
            ...
            for i in range(1, 11):
                if self.dataset.loc[indx, f'home_team_win_{i}'] == 1 and self.dataset.loc[
                    indx, f'home_team_history_is_play_home_{i}'] == 0:
                    summ += 1
            self.dataset.loc[indx, 'home_team_perc_win_away'] = summ / (
                        10 - self.dataset.loc[indx, 'home_count_home_games'])
        ...
        for indx in tqdm(self.dataset.index):
            ...
            for i in range(1, 11):
                if self.dataset.loc[indx, f'away_team_win_{i}'] == 1 and self.dataset.loc[
                    indx, f'away_team_history_is_play_home_{i}'] == 0:
                    summ += 1
            self.dataset.loc[indx, 'away_team_perc_win_away'] = summ / (
                        10 - self.dataset.loc[indx, 'away_count_home_games'])
        ...
        for indx in tqdm(self.dataset.index):
            ...
            for i in range(1, 11):
                if self.dataset.loc[indx, f'home_team_win_{i}'] == 1 and self.dataset.loc[
                    indx, f'home_team_history_is_cup_{i}'] == 1:
                    summ += 1
        ...
        for indx in tqdm(self.dataset.index):
            ...
            for i in range(1, 11):
                if self.dataset.loc[indx, f'away_team_win_{i}'] == 1 and self.dataset.loc[
                    indx, f'away_team_history_is_cup_{i}'] == 1:
                    summ += 1
    ...
    def Ft_eng_rating(self):
        ...
        home_league = self.dataset[['league_name', 'home_team_mean_rating']].reset_index().drop(['id'], axis=1).rename(
            columns={'league_name': 'league_name', "home_team_mean_rating": "rating"})
        away_league = self.dataset[['league_name', 'away_team_mean_rating']].reset_index().drop(['id'], axis=1).rename(
            columns={'league_name': 'league_name', "away_team_mean_rating": "rating"})
        all_team_league = pd.concat([home_league, away_league])
        self.dataset['mean_rating_in_league'] = self.dataset['league_name'].map(
            all_team_league.groupby(['league_name'])['rating'].mean().to_dict())

        # diff = team_rating - mean_rating_league
        self.dataset['home_team_diff_rating_league'] = self.dataset['home_team_mean_rating'] - self.dataset[
            'mean_rating_in_league']
        self.dataset['away_team_diff_rating_league'] = self.dataset['away_team_mean_rating'] - self.dataset[
            'mean_rating_in_league']

        # mean rating each team
        home_team = self.dataset[['home_team_name', 'home_team_mean_rating']].reset_index().drop(['id'], axis=1).rename(
            columns={'home_team_name': 'team_name', "home_team_mean_rating": "rating"})
        away_team = self.dataset[['away_team_name', 'away_team_mean_rating']].reset_index().drop(['id'], axis=1).rename(
            columns={'away_team_name': 'team_name', "away_team_mean_rating": "rating"})
        all_team_team = pd.concat([home_team, away_team])
        self.dataset['home_team_mean_rating_general'] = self.dataset['home_team_name'].map(
            all_team_team.groupby('team_name')['rating'].mean().to_dict())
        self.dataset['away_team_mean_rating_general'] = self.dataset['away_team_name'].map(
            all_team_team.groupby('team_name')['rating'].mean().to_dict())
        ...
        # ELO_rating
        for i in range(1, 11):
            self.dataset[f'home_team_history_ELO_rating_{i}'] = 1 / (1 + 10 ** ((self.dataset[
                                                                                     f'home_team_history_opponent_rating_{i}'] -
                                                                                 self.dataset[
                                                                                     f'home_team_history_rating_{i}']) / 10))
            self.dataset[f'away_team_history_ELO_rating_{i}'] = 1 / (1 + 10 ** ((self.dataset[
                                                                                     f'away_team_history_opponent_rating_{i}'] -
                                                                                 self.dataset[
                                                                                     f'away_team_history_rating_{i}']) / 10))

        self.dataset['home_team_ELO_rating'] = self.dataset[
            [f'home_team_history_ELO_rating_{i}' for i in range(1, 11)]].mean(axis=1)
        self.dataset['away_team_ELO_rating'] = self.dataset[
            [f'away_team_history_ELO_rating_{i}' for i in range(1, 11)]].mean(axis=1)
    ...
    def Ft_eng_league_id(self):
        ...
        self.dataset['home_team_count_league'] = self.dataset[['league_id'] + home_team_history_league].apply(
            lambda x: len(set(x)), axis=1)
        self.dataset['away_team_count_league'] = self.dataset[['league_id'] + away_team_history_league].apply(
            lambda x: len(set(x)), axis=1)

        self.dataset['home_team_same_league'] = self.dataset['home_team_count_league'].apply(lambda x: x == 1).map(
            {True: 1, False: 0})
        self.dataset['away_team_same_league'] = self.dataset['away_team_count_league'].apply(lambda x: x == 1).map(
            {True: 1, False: 0})
    ...
    def Ft_eng_coach_id(self):
        ...
        self.dataset['home_team_count_coach'] = self.dataset[['home_team_coach_id'] + home_team_history_coach].apply(
            lambda x: len(set(x)), axis=1)
        self.dataset['away_team_count_coach'] = self.dataset[['away_team_coach_id'] + home_team_history_coach].apply(
            lambda x: len(set(x)), axis=1)

        self.dataset['home_team_same_coach'] = self.dataset['home_team_count_coach'].apply(lambda x: x == 1).map(
            {True: 1, False: 0})
        self.dataset['away_team_same_coach'] = self.dataset['home_team_count_coach'].apply(lambda x: x == 1).map(
            {True: 1, False: 0})
